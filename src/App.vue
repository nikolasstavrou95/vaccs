<template>
  <v-app id="inspire">
    ...
    <v-footer  fluid padless class="blue-grey lighten-3 " >
    <v-col
      class="text-center"
      cols="12"
    >
      {{ new Date().getFullYear() }} — <strong>SKG code -Team C</strong>
    </v-col>
  </v-footer>
  </v-app>
</template>
...
<script>
  ...
  export default {
    data: () => ({ 
      drawer: null,
    items: [
          { title: 'Vaccinations', icon: 'mdi-home', to:'/' },
          { title: 'Patients', icon: 'mdi-account-multiple', to:'/patients' },
          { title: 'Vaccines', icon: 'mdi-clipboard-edit-outline', to:'/vaccines' },
          { title: 'Statistics', icon: 'mdi-chart-areaspline' , to:'/statistics'},
